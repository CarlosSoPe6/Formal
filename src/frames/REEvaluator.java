...
/**Class that evaluates a file. Contains methods to do so both from console or from a text file.*/
public class REEvaluator {
	...
	/**Set containing all strings accepted by the language. Helps to avoid repeated strings.*/
	static HashSet<String> answers=new HashSet<>();

	/**Initial call that evaluates if a string is accepted by the NFD
	 * @param automata The NFD matrix generated.
...
//			if(accepted) break;
	...
	/**Helper method that recurs through states in order to test all possible paths to make decision if string is accepted.
	 * @param automata The NFD matrix representation.
	 * @param string The string to be evaluated.
	 * @param currentState The current State represented as a column value(int).
	 * @param charAt The current amount of characters processed from string.
...
//			if(accepted) break;
		...
		//After it evaluates a string for every accepted character, it searches for a wildcard symbol transition to accept any input for one transition:
		LinkedList<Integer> wildcardStates=automata[currentState]['&'];
		for(int nextState: wildcardStates) {
			if(charAt+1<string.length())	{
				accepted=isAcceptedRecursive(automata, string,nextState,charAt+1);
...
//			if(accepted) break;
	...
	/**Method that evaluates a regular expression and tests string directly from console input.
	 * The input must contain a RegEx in the first line, followed by n strings to be tested. Last line must have a -1 input.*/
	...
	/**Method that evaluates a regular expression and tests string directly from txt file.
	 * The input must contain a RegEx in the first line, followed by n strings to be tested.*/
